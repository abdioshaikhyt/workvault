...
CELERY_TASK_ROUTES = {
    'backend2app.tasks.*': {'queue': 'backend2'},
}
CELERY_BEAT_SCHEDULE = {
    'flush-blacklisted-access-tokens-daily': {
        'task': 'backend2app.tasks.delete_all_logout_tokens',
        'schedule': crontab(hour=23, minute=59),
        'options': {'queue': 'backend2'},
    },
}
...
hosts_env = os.getenv('ELASTICSEARCH_DSL_HOSTS', '')
hosts_list = hosts_env.split()

ELASTICSEARCH_DSL = {
    'default': {
        'hosts': hosts_list,
        'http_auth': (
            os.getenv('ELASTICSEARCH_DSL_HTTP_AUTH_USER'),
            os.getenv('ELASTICSEARCH_DSL_HTTP_AUTH_PASSWORD'),
        ),
        'verify_certs': False,
    }
}
